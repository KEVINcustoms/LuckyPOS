...
public class product extends javax.swing.JPanel {
    ...
    private void createActionsPanel() {
        ...
        import_invoice.setToolTipText("Import a supplier invoice with Azure AI or offline Tesseract OCR, then review before receiving stock");
    ...
    private void importInvoiceActionPerformed() {
        if (conn == null) {
            JOptionPane.showMessageDialog(this, "Database connection is not available. Please restart LuckyPOS and try again.", "Import Invoice", JOptionPane.WARNING_MESSAGE);
            return;
        }
        final java.awt.Window owner = SwingUtilities.getWindowAncestor(this);
        SwingUtilities.invokeLater(() -> {
            try {
                final java.awt.Window ownerWindow = owner == null ? null : owner;
                final Connection connection = conn;
                com.nexatek.invoice.InvoiceImportDialog dialog = new com.nexatek.invoice.InvoiceImportDialog(ownerWindow, connection, this::Update_table);
                dialog.setLocationRelativeTo(ownerWindow);
                dialog.setVisible(true);
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Unable to open the invoice import window: " + ex.getMessage(),
                        "Import Invoice", JOptionPane.ERROR_MESSAGE);
            }
        });
    }
